  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      const newEntry: KnowledgeEntry = {
        ...
        topic,
        content,
        type: topic.includes('Treinamento') ? 'training' : 'rule',
        active: true
      };
    ...
    <div className="p-8 h-full overflow-y-auto custom-scrollbar bg-zinc-950">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-1 space-y-6">
          <div className={`p-8 rounded-[40px] border transition-all sticky top-0 ${editingId ? 'border-blue-500/30 bg-blue-500/[0.02]' : 'border-zinc-800 bg-zinc-900/20'}`}>
            <div className="flex justify-between items-center mb-6">
               ...
               <button 
                 ...
                 className="text-[9px] font-black uppercase text-yellow-400 bg-yellow-400/10 px-3 py-1.5 rounded-lg border border-yellow-400/20 hover:bg-yellow-400 hover:text-black transition-all"
               >
                 ✨ Template Livro
               </button>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Ex: Regra de Promoção" className={`w-full bg-zinc-900 border rounded-2xl px-6 py-4 focus:outline-none transition-all text-sm border-zinc-800 focus:border-yellow-400 text-white`} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Instrução Completa</label>
                <textarea value={content} onChange={e => setContent(e.target.value)} rows={10} placeholder="Descreva o que o Nobelino deve saber..." className={`w-full bg-zinc-900 border rounded-2xl px-6 py-4 focus:outline-none transition-all text-sm resize-none border-zinc-800 focus:border-yellow-400 text-white`} />
              </div>
              ...
              <div className="flex flex-col gap-3 pt-2">
                ...
                {editingId && <button onClick={clearForm} className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500">Cancelar</button>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
           ...
           <div className="grid grid-cols-1 gap-6">
              ...
              {entries.map(entry => (
                <div key={entry.id} className={`p-8 bg-zinc-900/30 border rounded-[40px] group relative transition-all border-zinc-800 hover:bg-zinc-900/50 ${entry.topic.includes('Treinamento') ? 'border-blue-500/10' : ''}`}>
                   <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-3">
                         <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg ${entry.topic.includes('Treinamento') ? 'bg-blue-500/10 text-blue-400' : 'bg-yellow-400/10 text-yellow-400'}`}>
                            {entry.topic.includes('Treinamento') ? '🧠' : '📜'}
                         </div>
                         <div>
                            ...
                            <p className="text-[8px] font-black text-zinc-600 uppercase tracking-widest mt-2">
                               Tipo: {entry.topic.includes('Treinamento') ? 'Treinamento via Chat' : 'Manual de Balcão'}
                            </p>
                         </div>
                      </div>
